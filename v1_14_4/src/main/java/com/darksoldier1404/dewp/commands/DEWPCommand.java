package com.darksoldier1404.dewp.commands;
...
public class DEWPCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if(args[0].equalsIgnoreCase("생성")) {
            ...
            if(args.length == 2) {
                DEWPFunction.createWarp(p, args[1]);
                return false;
            }
        }
        if(args[0].equalsIgnoreCase("링크")) {
            ...
            if(args.length == 3) {
                DEWPFunction.linkWarp(p, args[1], args[2]);
                return false;
            }
        }
        if(args[0].equalsIgnoreCase("삭제")) {
            ...
            if(args.length == 2) {
                DEWPFunction.deleteWarp(p, args[1]);
                return false;
            }
        }
        if(args[0].equalsIgnoreCase("목록")) {
            DEWPFunction.listWarp(p);
            return false;
        }
        if(args[0].equalsIgnoreCase("티피")) {
            ...
            if(args.length == 2) {
                DEWPFunction.teleportToWarp(p, args[1]);
                return false;
            }
        }
        if(args[0].equalsIgnoreCase("리로드")) {
            DEWPFunction.reloadConfig();
